...
function Game() {
	...
	function scoreReducer(arr, action, id) {
		...
		const newdisArray = [ ...state.disArray.slice(0, id), !state.disArray[id], ...state.disArray.slice(id + 1) ];
	...
	function handleClick(idx) {
		...
		if (state.rollsLeft > 0 && !state.rolling) {
			const newLock = [ ...state.locked.slice(0, idx), !state.locked[idx], ...state.locked.slice(idx + 1) ];
		...
		<myContext.Provider value={{ state, setState, handleClick: handleClick, scoreReducer: scoreReducer }}>
			{state.count < 13 ? (
				...
				<div className="gameover">
					<h1>Game Over</h1>
					<h2 className="h2score">you scored {state.total}</h2>
				</div>
			)}
		</myContext.Provider>
	);
